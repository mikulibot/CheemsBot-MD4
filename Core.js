...
if (isBanChat) return reply(mess.bangc)
 ...
 case 'منشن': case 'tagall': case 'تاك': case 'تاج':{
    if (isBan) return reply(mess.banned)	 			
 if (isBanChat) return reply(mess.bangc)
 if (!m.isGroup) return replay(mess.grouponly)
 if (!isAdmins && !isCreator) return replay(mess.useradmin)
 ...
 *الرساله : ${args.join(" ") ? args.join(" ") : 'لا يوجد رساله'}*\n\n`
...
let mentioned = participants.map(v => v.jid)
 ...
 case 'resetlinkgc':
    ...
    case 'group': case 'جروب': {
     ...
     case 'demote': case 'تنزيل': case 'اقاله': case 'اقالة':{
        if (isBan) return reply(mess.banned)	 			
     if (isBanChat) return reply(mess.bangc)
     if (!m.isGroup) return replay(mess.grouponly)
     if (!isBotAdmins) return replay(mess.botadmin)
     ...
     case 'remove': case 'طرد': case 'اطرد': case 'اذلف':{
        if (isBan) return reply(mess.banned)	 			
     if (isBanChat) return reply(mess.bangc)
     if (!m.isGroup) return replay(mess.grouponly)
     if (!isBotAdmins) return replay(mess.botadmin)
...
انا *🔱 𝚁𝚊𝚒𝚣𝚎𝚕 𝙶𝚎𝚘𝚛𝚐𝚎 🔱️*, انا بوت مصنوع من قبل *رايزل جورج❤️*/ أنا * RAIZEL ᗷOT❤️ * ، روبوت تم تطويره بواسطة * RAIZEL GORGE❤️ *.
...
🍁 Then type/ثم اكتب " *${prefix}قائمه.الاباحي* " للحصول على قائمة كاملة بأوامر الاباحي.




 『  *${global.BotName}*  』
 Powered by: *RAIZEL/تم التطوير بواسطه:رايزل*
`
...
case 'add': case 'اضافه': case 'اضافة': case 'هاتو':{     			
    if (!m.isGroup) return replay(mess.grouponly)
 if (!isBotAdmins) return replay(mess.botadmin)
...
default:
    ...
    if(isCmd){
        if (isBan) return reply(mess.banned)	 			
        if (isBanChat) return reply(mess.bangc)
        reply (`لا يوجد امر مبرمج مثل هذا *${pushname}* /اكتب *${prefix}help/الاوامر* للحصول على قائمة الأوامر الكاملة الخاصة بي!`)
